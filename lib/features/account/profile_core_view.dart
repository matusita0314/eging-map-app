...
// /// プロフィール画面の共通UIとロジックをまとめたウィジェット
// class ProfileCoreView extends StatefulWidget {
//   final String userId;
//   /// ページ固有のアクションボタン（編集やチャットなど）を構築するための関数
//   final Widget Function(DocumentSnapshot userDoc) actionButtonsBuilder;

//   const ProfileCoreView({
//     super.key,
...
//   @override
//   State<ProfileCoreView> createState() => _ProfileCoreViewState();
// }

// class _ProfileCoreViewState extends State<ProfileCoreView> {
...
//               // ここでページ固有のアクションボタンを構築して表示する
//               widget.actionButtonsBuilder(userDoc),
...
//                 child: Text('これまでの投稿', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
//               ),
//               _buildUserPostsGrid(),
//             ],
//           ),
...
//   // --- 以下、UIを構築するメソッド群（内容はaccount.dartからそのままコピー） ---
...
//         children: [
//           CircleAvatar(radius: 40, backgroundImage: photoUrl.isNotEmpty ? NetworkImage(photoUrl) : null, child: photoUrl.isEmpty ? const Icon(Icons.person, size: 40) : null),
...
//                 Text(introduction, style: const TextStyle(fontSize: 16, height: 1.4), maxLines: 2, overflow: TextOverflow.ellipsis),
//               ],
//             ),
//           ),
//         ],
...
//     String badgeText;
//     switch (rank) {
//       case 'amateur': badgeColor = Colors.blue.shade700; badgeText = 'アマチュア'; break;
//       case 'pro': badgeColor = Colors.amber.shade800; badgeText = 'プロ'; break;
//       default: badgeColor = Colors.green; badgeText = 'ビギナー';
//     }
//     return Container(
...
//         _buildTappableStatItem('保存', 'saved_posts', SavedPostsPage(userId: widget.userId)),
//         _buildTappableStatItem('フォロワー', 'followers', FollowerListPage(userId: widget.userId, listType: FollowListType.followers)),
//         _buildTappableStatItem('フォロー中', 'following', FollowerListPage(userId: widget.userId, listType: FollowListType.following)),
//       ],
//     );
//   }

//   Widget _buildTappableStatItem(String label, String collectionPath, Widget destinationPage) {
//     return InkWell(
...
//         Text('バッジ', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
//         SizedBox(height: 8),
...
//       child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
//         const Text('月別釣果グラフ', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
//         const SizedBox(height: 16),
//         SizedBox(
//           height: 200,
...
//               show: true,
//               bottomTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, getTitlesWidget: (value, meta) => Text(sortedEntries[value.toInt()].key.substring(5), style: const TextStyle(fontSize: 10)), reservedSize: 30)),
...
//               return BarChartGroupData(x: index, barRods: [BarChartRodData(toY: sortedEntries[index].value.toDouble(), color: Colors.blue, width: 16, borderRadius: BorderRadius.circular(4))]);
//             }),
//           )),
//         ),
//       ]),
...
//         return PostGridCard(post: post, isLikedByCurrentUser: _likedPostIds.contains(post.id), isSavedByCurrentUser: _savedPostIds.contains(post.id), isFollowingAuthor: _followingUserIds.contains(post.userId));
//       },
//     );
//   }
// }