...
class EditTournamentPostPage extends StatefulWidget {
  ...
  const EditTournamentPostPage({super.key, required this.tournament, required this.post});
...
class _EditTournamentPostPageState extends State<EditTournamentPostPage> {
  ...
  final _tackleRodController_existing = TextEditingController(); // 既存のコントローラーと名前が衝突しないように変更
  final _tackleReelController_existing = TextEditingController(); // 既存のコントローラーと名前が衝突しないように変更
  final _tackleLineController_existing = TextEditingController(); // 既存のコントローラーと名前が衝突しないように変更
  ...
  void _loadPostData() {
    ...
    if (metric == 'SIZE' || metric == 'COUNT') {
      ...
      _tackleRodController_existing.text = postData['tackleRod'] ?? '';
      _tackleReelController_existing.text = postData['tackleReel'] ?? '';
      _tackleLineController_existing.text = postData['tackleLine'] ?? '';
      ...
      _waterTemperature = (postData['waterTemperature'] as num? ?? 15.0).toDouble();
      }
    }
  }
  ...
  void dispose() {
    ...
    if (_existingImageUrls.length + _newImageBytes.length >= maxImages) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('画像は$maxImages枚までです。')));
      return;
    }

    // --- ルールに応じてカメラかギャラリーかを判定 ---
  ...
  Future<void> _updatePost() async {
    ...
    try {
      ...
      for (final url in _deletedImageUrls) {
        if (url.isNotEmpty) await FirebaseStorage.instance.refFromURL(url).delete();
      }

      // 2. 新しい画像をStorageにアップロードしてURLを取得
      for (final bytes in _newImageBytes) {
        final fileName = '${DateTime.now().millisecondsSinceEpoch}.jpg';
        final ref = FirebaseStorage.instance.ref('tournaments/${widget.tournament.id}/${_currentUser.uid}/$fileName');
      ...
      if (metric == 'SIZE' || metric == 'COUNT') {
        updateData.addAll({
          ...
          'tackleRod': _tackleRodController_existing.text,
          'tackleReel': _tackleReelController_existing.text,
          'tackleLine': _tackleLineController_existing.text,
        ...
        if (widget.tournament.name.contains("料理")) {
          updateData['ingredients'] = _ingredientControllers
              ...
              .map((c) => {'name': c.nameController.text, 'quantity': c.quantityController.text})
              .toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('${widget.tournament.name} の編集')),
      body: SingleChildScrollView(
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              _buildSectionTitle('画像 (${widget.tournament.rule.maxImageCount}枚まで)'),
              _buildImagePicker(),
              const SizedBox(height: 24),
              // 大会の種類に応じて表示するフォームを切り替え
              if (widget.tournament.rule.metric == 'SIZE' || widget.tournament.rule.metric == 'COUNT')
                _buildMainTournamentForm()
              else if (widget.tournament.rule.metric == 'LIKE_COUNT')
                if (widget.tournament.name.contains("料理"))
  ...
  // 以下、フォーム構築用のヘルパーメソッド (add_tournament_post_page.dartから流用)
  Widget _buildImagePicker() {
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          if (index == totalImages) {
            return totalImages < maxImages ? _buildAddImageButton() : const SizedBox.shrink();
          }
          // 既存・新規画像の表示
          Widget imageWidget;
          bool isExistingImage = index < _existingImageUrls.length;
          if (isExistingImage) {
            imageWidget = CachedNetworkImage(imageUrl: _existingImageUrls[index], fit: BoxFit.cover, width: 100, height: 100);
          } else {
            imageWidget = Image.memory(_newImageBytes[index - _existingImageUrls.length], fit: BoxFit.cover, width: 100, height: 100);
          }
    ...
    Widget _buildImageTile(Widget image, bool isExisting, int index, VoidCallback onDeleted) {
    return SizedBox(
      width: 100,
      height: 100,
      child: Stack(
      ...
      children: [
        ...
        if (widget.tournament.rule.metric == 'COUNT') ...[
          TextFormField(
            ...
            decoration: const InputDecoration(labelText: '匹数 *', border: OutlineInputBorder()),
  ...
  Widget _buildCookingContestForm() {
    // add_tournament_post_page.dartの同名メソッドを流用
     return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        ...
        ListView.builder(
          ...
          itemBuilder: (context, index) {
            return Padding(
              ...
              child: Row(children: [
                Expanded(flex: 2, child: TextFormField(controller: _ingredientControllers[index].nameController, decoration: const InputDecoration(labelText: '材料名', border: OutlineInputBorder()))),
                const SizedBox(width: 8),
                Expanded(flex: 2, child: TextFormField(controller: _ingredientControllers[index].quantityController, decoration: const InputDecoration(labelText: '分量', border: OutlineInputBorder()))),
                if (_ingredientControllers.length > 1) IconButton(icon: const Icon(Icons.remove_circle_outline),onPressed: () => setState(() { _ingredientControllers[index].dispose(); _ingredientControllers.removeAt(index); })) else const SizedBox(width: 48),
              ]),
            );
          },
        ),
        TextButton.icon(icon: const Icon(Icons.add), label: const Text('材料を追加'), onPressed: () => setState(() => _ingredientControllers.add(IngredientController()))),
        const SizedBox(height: 16),
        _buildSectionTitle('作り方など'),
        TextFormField(controller: _processController, decoration: const InputDecoration(labelText: '調理工程 (任意)', border: OutlineInputBorder(), alignLabelWithHint: true), maxLines: 10),
        const SizedBox(height: 16),
        TextFormField(controller: _impressionController, decoration: const InputDecoration(labelText: '感想 (任意)', border: OutlineInputBorder(), alignLabelWithHint: true), maxLines: 3),
      ],
    );
  }
    ...
    // add_tournament_post_page.dartの同名メソッドを流用
     return Column(
      ...
      children: [
        ...
        TextFormField(controller: _tackleRodController, decoration: const InputDecoration(labelText: 'ロッド *', border: OutlineInputBorder()), validator: (v) => (v == null || v.isEmpty) ? '必須項目です' : null),
        const SizedBox(height: 16),
        TextFormField(controller: _tackleReelController, decoration: const InputDecoration(labelText: 'リール *', border: OutlineInputBorder()), validator: (v) => (v == null || v.isEmpty) ? '必須項目です' : null),
        const SizedBox(height: 16),
        TextFormField(controller: _tackleLureController, decoration: const InputDecoration(labelText: 'ルアー *', border: OutlineInputBorder()), validator: (v) => (v == null || v.isEmpty) ? '必須項目です' : null),
        const SizedBox(height: 16),
        TextFormField(controller: _tackleLineController, decoration: const InputDecoration(labelText: 'ライン *', border: OutlineInputBorder()), validator: (v) => (v == null || v.isEmpty) ? '必須項目です' : null),
        const SizedBox(height: 16),
        TextFormField(controller: _appealPointController, decoration: const InputDecoration(labelText: 'アピールポイント *', border: OutlineInputBorder(), alignLabelWithHint: true), maxLines: 5, validator: (v) => (v == null || v.isEmpty) ? '必須項目です' : null),
      ],
    );
  }
